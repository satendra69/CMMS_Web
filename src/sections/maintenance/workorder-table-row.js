...
export default function WorkOrderTableRow({
  ...
  const filteredData = TableStatus.filter(item => item.site_cd === empl_site_cd && item.wrk_sts_status === col6);

  const maxCharactersToShow = 30;
  const truncatedDescription =
  col7 && col7.length > maxCharactersToShow
      ? `${col7.substring(0, maxCharactersToShow)}...`
      : col7;
        ...
        <TableRow hover selected={selected}>
          <TableCell align="right">
            <IconButton
              color={popover.open ? "primary" : "default"}
              onClick={popover.onOpen}
              disableRipple // Disable the click-and-hold effect
          ...
          <TableCell>
            <Tooltip title={col7} placement="top" arrow >
              <span>{truncatedDescription}</span>
            </Tooltip>
          </TableCell>
          <TableCell>{col8}</TableCell>
          <TableCell>{col9 ? formatDate2(col9) : ""}</TableCell>
          <TableCell>{col10 ? formatDate(col10) : ""}</TableCell>
          <TableCell>{col11 ? formatDate2(col11) : ""}</TableCell>
          <TableCell>{col12 ? formatDate2(col12) : ""}</TableCell>
