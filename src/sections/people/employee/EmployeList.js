...
const [fetch,setRefetch]=useState(false)
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setLastQuery(response.data.employe_result.result);
      // Swal.close();
      // setIsLoading(false);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  }, [site_ID, currentPage,fetch]);
...
const fetchDataResponse = async (hasRowIdValuept) => {
  try {
    ...
    console.log("response_save",response)
...
const handleDropDownPromptSaveAsBtn = async () => {
  ...
  if (hasEmptyValuept) {
    ...
    // Select DroupDown Value employe list Filter end
  ...
  const handleOptionChange = async (event, responseData) => {
    ...
    if(GetPrompt === '1'){
      ...
      try {
        ...
        console.log("response_emp_filter",response)
    ...
    if(selectedValue){
      Swal.fire({
        ...
        setTableData(lastQuery);
        setTotalRow(lastQuery.length);
    ...
    if(selectedValue==="001. ACT ID"){
      ...
      try {
        ...
        const response= await httpCommon.get(`get_emp_filter_act.php?site_cd=${site_ID}`)
...
console.log("WorkReqFiledname",WorkReqFiledname)
...
setRefetch2(true)
  ...
  const handelSearchButton = async () => {
    ...
    //       emp_mst.emp_mst_name LIKE 'SATYA_EMP_ID'
    //     OR emp_mst.emp_mst_title LIKE 'SATYA_EMP_ID'
    //     OR emp_mst.emp_mst_status LIKE 'SATYA_EMP_ID'
    //     OR emp_mst.emp_mst_homephone LIKE 'SATYA_EMP_ID'
    //     OR emp_mst.emp_mst_emg_name LIKE 'SATYA_EMP_ID'
    //     OR emp_mst.emp_mst_emg_phone LIKE 'SATYA_EMP_ID'
		// OR emp_mst.emp_mst_empl_id LIKE 'SATYA_EMP_ID'
		// OR emp_mst.emp_mst_login_id LIKE 'SATYA_EMP_ID'
                          ...
                          <tr key={index}>
                            ...
                            <td style={{ width: "25%" }}>
                              <Select
                                ...
                                value={row && row.selectedOption?row.selectedOption:"" }
                                onChange={(event) =>
