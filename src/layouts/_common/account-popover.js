...
export default function AccountPopover({ onRowClick }) {
  ...
  const Emp_mst_empl_id = localStorage.getItem("emp_mst_login_id");
  ...
  const fetchImgData = async () => {
    ...
    //  console.log("response__img11", response);
      if (response.data.data.UserProfileDt.length > 0) {
        //console.log("response__img",response.data.data.AllRef.length);
    ...
    if (buttonName === "On Shift") {
      ...
      try {
        ...
        if (response.data.status === "SUCCESS") {
          ...
          // toast.success("User Status Update is Successfully!", {
          //   position: "top-center",
          //   autoClose: 2000,
